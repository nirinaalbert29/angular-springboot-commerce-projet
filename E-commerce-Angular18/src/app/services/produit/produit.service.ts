...
export class ProduitService {
  ...
  adorer(produitId:number,token:string):Observable<Produit>{
    const url = `${this.BASE_URL}/user/produit/like/${produitId}`;
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`
    });
    return this.http.put<Produit>(url,null,{ headers });
  }

  etatLike(id:number,token:string):Observable<boolean>{
    const url = `${this.BASE_URL}/user/produit/likeState/${id}`;
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`
    });
